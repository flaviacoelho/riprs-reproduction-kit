...
def summarizing_results(data):
    ...
    for pair_c, group_c in data_g:
        ...
        if (count_index == N_STRATUM):
            print('Number of RIPRs induced by refactoring edits - by level:',count_induced_by_level)
            count_induced_by_level = 0
            count_index = 0
    print('Number of RIPRs induced by refactoring edits:',count_induced)
    return

print('Summary of RIPRs induced by refactoring edits for our stratified sample')
display_bar()
...
def summarizing_by_category(data, level):
    display_bar()
    print('PRs containing a', level, 'number of refactoring operations')
    summarizing(data)
    return
